...
const KIND_COLOR: Record<string, string> = {
  "Rückblick": "bg-primary/15 text-primary",
  "Kernpunkte": "bg-warning/15 text-warning",
  "KI-Frage": "bg-success/15 text-success",
};
...
export function Notes() {
  ...
  const fmt = (ts: number) => new Date(ts).toLocaleString("de-DE", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" });

  return (
    <div>
      <PageHeader
        title="Notizen"
        subtitle="KI-Rückblicke / Kernpunkte / Q&A lokal sammeln und jederzeit nachlesen. Daten nur lokal, kein Upload."
        actions={notes.length > 0 && (
          <button onClick={() => { if (confirm("Alle Notizen löschen?")) { clearNotes(); setNotes([]); } }}
      ...
      {notes.length === 0 ? (
        <GlassCard>
          <div className="flex flex-col items-center gap-2 py-10 text-center text-sm text-muted-foreground">
            ...
            Noch keine Notizen. Klicke im »Tagesrückblick«, »Nachrichten-Radar« oder bei »KI fragen« auf <b className="text-foreground">»Speichern«</b>, um Analyseergebnisse zu sichern.
          </div>
        </GlassCard>
      ) : (
            ...
            return (
              <GlassCard key={n.id} className="!p-0 overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-3">
                  ...
                  <button onClick={() => setNotes(deleteNote(n.id))} className="shrink-0 text-muted-foreground/60 hover:text-destructive" title="Löschen">
